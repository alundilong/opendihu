...
def save_fibers_as_vtp(streamlines, output_filename):
    """
    Save fiber streamlines as VTK PolyData.

    Each streamline becomes one polyline cell and receives:
        fiber_id:       zero-based fiber index
        fiber_number:   one-based fiber number
        n_points:       number of points in the fiber
    """
    ...
    for fiber_id, streamline in enumerate(streamlines):
        ...
        for p in streamline:
            point = np.asarray(p[:3], dtype=float)

            # Retain your original filtering condition
            if np.linalg.norm(point) < 1e-3:
                continue

            valid_points.append(point)

        # At least two points are needed to define a polyline
        if len(valid_points) < 2:
            print(
                f"Warning: fiber {fiber_id + 1} contains fewer than "
                f"2 valid points and was skipped."
            )
            continue
        ...
        # PyVista line-cell format:
        # [number_of_points, point_id_0, point_id_1, ...]
        point_ids = np.arange(
            point_offset,
        ...
        lines.append(
            np.concatenate(
                ([len(valid_points)], point_ids)
            )
        )

        fiber_ids.append(fiber_id)
        fiber_numbers.append(fiber_id + 1)
        fiber_point_counts.append(len(valid_points))

        point_offset += len(valid_points)

    if not all_points:
        raise RuntimeError("No valid fiber streamlines were found.")

    points_array = np.asarray(all_points, dtype=float)
    lines_array = np.concatenate(lines).astype(np.int64)

    fibers = pv.PolyData()
    fibers.points = points_array
    fibers.lines = lines_array

    # One value for each polyline cell
    fibers.cell_data["fiber_id"] = np.asarray(
        fiber_ids, dtype=np.int32
    )
    fibers.cell_data["fiber_number"] = np.asarray(
        fiber_numbers, dtype=np.int32
    )
    fibers.cell_data["n_points"] = np.asarray(
        fiber_point_counts, dtype=np.int32
    )

    fibers.save(output_filename)

    print(
        f'Saved {len(fiber_numbers)} fibers and '
        f'{len(points_array)} points to "{output_filename}".'
    )
...
print("                  {}.vtp".format(output_filename))
