...
void FastMonodomainSolverBase<nStates, nAlgebraics,
                              DiffusionTimeSteppingScheme>::run() {
  initialize();
  advanceTimeSpan();
}
...
void FastMonodomainSolverBase<
  ...
  LOG(DEBUG) << "durationLogKeys: " << durationLogKey0D_ << ","
             << durationLogKey1D_;
  ...
  for (int timeStepNo = 0; timeStepNo < nTimeStepsSplitting_; timeStepNo++) {
    ...
    LOG(DEBUG) << "splitting " << timeStepNo << "/" << nTimeStepsSplitting_
               << ", t: " << currentTime;
    ...
    bool storeAlgebraicsForTransfer =
        timeStepNo ==
        nTimeStepsSplitting_ -
            1; // after the last timestep, store the algebraics for transfer
...
void FastMonodomainSolverBase<
    nStates, nAlgebraics,
    DiffusionTimeSteppingScheme>::compute0D(double startTime,
                                            double timeStepWidth,
                                            int nTimeSteps,
                                            bool storeAlgebraicsForTransfer) {
  Control::PerformanceMeasurement::start(durationLogKey0D_);
  LOG(DEBUG) << "compute0D(" << startTime << "), " << nTimeSteps << " time step"
             << (nTimeSteps == 1 ? "" : "s");
  ...
  if (fiberData_.empty()) {
    LOG(DEBUG) << "In compute0D(" << startTime << "," << timeStepWidth << ","
               << nTimeSteps << "," << storeAlgebraicsForTransfer
               << "): fiberData_ is empty, nPointBuffers: " << nPointBuffers;
    LOG(DEBUG)
        << "This means there is no fiber to compute on this rank, they were "
           "all send to another rank for the computation. Skip compute0D.";
    return;
  }

  // set first and last point of the subdomain active to capture stimuli from
  // neighbors
  ...
  for (global_no_t pointBuffersNo = 0; pointBuffersNo < nPointBuffers;
    ...
    VLOG(3) << "currentPointIsInCenter: " << currentPointIsInCenter
            ...
            << ", indexInFiber:" << indexInFiber
            << ", fiberCenterIndex: " << fiberCenterIndex << ", "
            << (indexInFiber - fiberCenterIndex) << " < "
            << Vc::double_v::size();

    VLOG(3) << "pointBuffersNo: " << pointBuffersNo
            << ", fiberDataNo: " << fiberDataNo
            << ", indexInFiber: " << indexInFiber;
    ...
    if (disableComputationWhenStatesAreCloseToEquilibrium_) {
      for (int stateNo = 0; stateNo < nStates; stateNo++) {
        statesPreviousValues[stateNo] =
            fiberPointBuffers_[pointBuffersNo].states[stateNo];
      }
    }
    ...
    for (int timeStepNo = 0; timeStepNo < nTimeSteps; timeStepNo++) {
      ...
      if (currentPointIsInCenter)
        stimulateCurrentPoint = isCurrentPointStimulated(
            fiberDataNo, currentTime, currentPointIsInCenter);
      const bool argumentStoreAlgebraics =
          storeAlgebraicsForTransfer && timeStepNo == nTimeSteps - 1;

      // if the current point does not need to get computed because the value
      // won't change
      if (isEquilibriumAccelerationCurrentPointDisabled(stimulateCurrentPoint,
                                                        pointBuffersNo)) {
        continue;
      }

      // do not compute fiber if respective option is set and the fiber has not
      // yet been stimulated
      ...
      compute0DInstance_(
          fiberPointBuffers_[pointBuffersNo].states,
...
void FastMonodomainSolverBase<
    nStates, nAlgebraics,
    DiffusionTimeSteppingScheme>::compute1D(double startTime,
                                            double timeStepWidth,
                                            int nTimeSteps, double prefactor) {
  // if all entries are at equilibrium, nothing will be computed, skip also 1D
  // computation
  if (nFiberPointBufferStatesCloseToEquilibrium_ ==
      fiberPointBuffersStatesAreCloseToEquilibrium_.size()) {
  ...
  // Crank-Nicolson Implicit Euler step: (K - 1/dt*M) u^{n+1} = -1/dt*M u^{n})
  // Crank-Nicolson step:
  ...
  for (int fiberDataNo = 0; fiberDataNo < fiberData_.size(); fiberDataNo++) {
...
#ifndef NDEBUG
    VLOG(1) << "fiber " << fiberDataNo << "/" << fiberData_.size()
            << ", valuesOffset: " << fiberData_[fiberDataNo].valuesOffset
            << ", has " << nValues << " values: ";

    std::stringstream s, s2;
    for (int valueNo = 0; valueNo < nValues; valueNo++) {
      global_no_t valuesIndexAllFibers =
      ...
      s << u;
      s2 << "[" << pointBuffersNo << "][" << entryNo << "]("
         << valuesIndexAllFibers << ")  ";
    }
    VLOG(1) << s.str();
    ...
    for (int valueNo = 0; valueNo < nValues; valueNo++) {
      ...
      if (valueNo > 0) {
        global_no_t valuesIndexAllFibers =
            fiberData_[fiberDataNo].valuesOffset + valueNo - 1;
        global_no_t pointBuffersNo =
            valuesIndexAllFibers / Vc::double_v::size();
        int entryNo = valuesIndexAllFibers % Vc::double_v::size();
        ...
        } else // Crank-Nicolson
        {
          ...
          d += (-k_left / 2. - 1 / dt * m_left) * u_previous +
               (-k_right / 2. - 1 / dt * m_right) * u_center;
        }
      }
      ...
      if (valueNo < nValues - 1) {
        ...
        } else // Crank-Nicolson
        {
          ...
          d += (-k_left / 2. - 1 / dt * m_left) * u_center +
               (-k_right / 2. - 1 / dt * m_right) * u_next;
        }
      }
      ...
      if (valueNo == 0) {
        ...
        // d'_i = (d_i - d'_{i-1}*a_i) / (b_i - c'_{i-1}*a_i)
        dAlgebraic[valueNo] = (d - dAlgebraic[valueNo - 1] * a) /
                              (b - cAlgebraic[valueNo - 1] * a);
      }

#ifndef NDEBUG
      if (valueNo < 5 || valueNo >= nValues - 6) {
        VLOG(2) << "valueNo: " << valueNo << ", a: " << a << ", b: " << b
                << ", c: " << c << ", d: " << d << ", u: " << u_center
                << ", c': " << (valueNo < nValues - 1 ? cAlgebraic[valueNo] : 0)
                << ", d': " << dAlgebraic[valueNo] << " = (" << d << "-"
                << dAlgebraic[valueNo - 1] * a << ")/(" << b << "-"
                << cAlgebraic[valueNo - 1] * a
                << ") = " << (d - dAlgebraic[valueNo - 1] * a) << "/"
                << (b - cAlgebraic[valueNo - 1] * a);
      }
#endif
    }
    ...
    // x_n = d'_n
    global_no_t valuesIndexAllFibers =
        fiberData_[fiberDataNo].valuesOffset + nValues - 1;
    global_no_t pointBuffersNo = valuesIndexAllFibers / Vc::double_v::size();
    int entryNo = valuesIndexAllFibers % Vc::double_v::size();
    fiberPointBuffers_[pointBuffersNo].states[0][entryNo] =
        dAlgebraic[nValues - 1];
    // LOG(DEBUG) << "set entry (" << pointBuffersNo << "," << entryNo << ") to
    // " << dAlgebraic[nValues-1];
    ...
    for (int valueNo = nValues - 2; valueNo >= 0; valueNo--) {
      ...
      double resultValue =
          dAlgebraic[valueNo] - cAlgebraic[valueNo] * previousValue;
...
template <int nStates, int nAlgebraics, typename DiffusionTimeSteppingScheme>
bool FastMonodomainSolverBase<nStates, nAlgebraics,
                              DiffusionTimeSteppingScheme>::
    isCurrentPointStimulated(int fiberDataNo, double currentTime,
                             bool currentPointIsInCenter) {
  FiberData &fiberDataCurrentPoint = fiberData_[fiberDataNo];

  // there is a parallel piece of code to this one in
  // CellmlAdapter<>::checkCallbackStates, cellml/03_cellml_adapter.tpp

  // get time from with testing for stimulation is enabled
  const double lastStimulationCheckTime =
      fiberDataCurrentPoint.lastStimulationCheckTime;

  const double setSpecificStatesCallFrequency =
      fiberDataCurrentPoint.setSpecificStatesCallFrequency;
  const double setSpecificStatesRepeatAfterFirstCall =
      fiberDataCurrentPoint.setSpecificStatesRepeatAfterFirstCall;
  const double setSpecificStatesCallEnableBegin =
      fiberDataCurrentPoint.setSpecificStatesCallEnableBegin;

  const int &motorUnitNo = fiberDataCurrentPoint.motorUnitNo;
  std::vector<double> &setSpecificStatesFrequencyJitter =
      fiberDataCurrentPoint.setSpecificStatesFrequencyJitter;
  ...
  if (VLOG_IS_ON(1)) {
    VLOG(1) << "currentTime: " << currentTime
            << ", lastStimulationCheckTime: " << lastStimulationCheckTime
            << ", next time point: "
            << lastStimulationCheckTime +
                   1. / (setSpecificStatesCallFrequency + currentJitter);
    VLOG(1) << "setSpecificStatesCallFrequency: "
  ...
  if (currentTime >=
    ...
    if (VLOG_IS_ON(1)) {
      ...
      VLOG(1) << "check if stimulation is over: duration already: "
              << currentTime -
                     (lastStimulationCheckTime +
                      1. / (setSpecificStatesCallFrequency + currentJitter))
              << ", setSpecificStatesRepeatAfterFirstCall: "
              << setSpecificStatesRepeatAfterFirstCall;
    }

    // if current stimulation is over
    if (setSpecificStatesRepeatAfterFirstCall != 0 &&
        currentTime - (lastStimulationCheckTime +
                       1. / (setSpecificStatesCallFrequency + currentJitter)) >
            setSpecificStatesRepeatAfterFirstCall) {
      // advance time of last call to specificStates
#ifndef NDEBUG
      LOG(DEBUG) << " old lastStimulationCheckTime: "
                 << fiberDataCurrentPoint.lastStimulationCheckTime
                 << ", currentJitter: " << currentJitter << ", add "
                 << 1. / (setSpecificStatesCallFrequency + currentJitter);
#endif
...
#ifndef NDEBUG
      LOG(DEBUG) << " new lastStimulationCheckTime: "
                 << fiberDataCurrentPoint.lastStimulationCheckTime;
#endif
      ...
      if (setSpecificStatesFrequencyJitter.size() > 0)
        jitterFactor = setSpecificStatesFrequencyJitter
            [jitterIndex % setSpecificStatesFrequencyJitter.size()];
      currentJitter = jitterFactor * setSpecificStatesCallFrequency;

#ifndef NDEBUG
      LOG(DEBUG) << " jitterIndex: " << jitterIndex
                 << ", new jitterFactor: " << jitterFactor
                 << ", currentJitter: " << currentJitter;
#endif
  ...
  // instead of calling setSpecificStates, directly determine whether to
  // stimulate from the firingEvents file
  int firingEventsIndex = round(currentTime * setSpecificStatesCallFrequency);

  bool stimulate =
      checkStimulation &&
      firingEvents_[firingEventsIndex % firingEvents_.size()]
                   [motorUnitNo %
                    firingEvents_[firingEventsIndex % firingEvents_.size()]
                        .size()];
  ...
  if (checkStimulation && VLOG_IS_ON(1)) {
    VLOG(1)
        << "setSpecificStatesCallFrequency: " << setSpecificStatesCallFrequency
        << ", firingEventsIndex: " << firingEventsIndex << ", fires: "
        << firingEvents_[firingEventsIndex % firingEvents_.size()]
                        [motorUnitNo %
                         firingEvents_[firingEventsIndex % firingEvents_.size()]
                             .size()];
  ...
  if (stimulate && currentPointIsInCenter) {
    // if this is the first point in time of the current stimulation, log
    // stimulation time
    if (!fiberDataCurrentPoint.currentlyStimulating) {
      fiberDataCurrentPoint.currentlyStimulating = true;
      Control::StimulationLogging::logStimulationBegin(
          currentTime, fiberDataCurrentPoint.motorUnitNo,
          fiberDataCurrentPoint.fiberNoGlobal);
    }

#ifndef NDEBUG
    LOG(DEBUG) << "stimulate fiber " << fiberDataCurrentPoint.fiberNoGlobal
               << ", MU " << motorUnitNo << " at t=" << currentTime;
    LOG(DEBUG)
        << "  motorUnitNo: " << motorUnitNo << " ("
        << motorUnitNo %
               firingEvents_[firingEventsIndex % firingEvents_.size()].size()
        << ")";
    LOG(DEBUG) << "  firing events index: " << firingEventsIndex << " ("
               << firingEventsIndex % firingEvents_.size() << ")";
    LOG(DEBUG) << "  setSpecificStatesCallEnableBegin: "
               << setSpecificStatesCallEnableBegin
               << ", lastStimulationCheckTime: " << lastStimulationCheckTime
               << ", stimulation already for "
               << +1. / (setSpecificStatesCallFrequency + currentJitter);
#endif

    LOG(INFO) << "t: " << currentTime << ", stimulate fiber "
              << fiberDataCurrentPoint.fiberNoGlobal << ", MU " << motorUnitNo;
  }
...
void FastMonodomainSolverBase<nStates, nAlgebraics,
                              DiffusionTimeSteppingScheme>::
    equilibriumAccelerationUpdate(const Vc::double_v statesPreviousValues[],
                                  int pointBuffersNo) {
  // every point is one of three possible states:
  // inactive:            do not check if the value changed, "inactive" can only
  // be set to "neighbor_is_active" by the neighbour point neighbor_is_active:
  // one of the neighbors is "active",
  //                      check if one of the neighbors is still "active",
  //                                 if not, change to "inactive"
  //                      enable check if the value changed,
  //                                 if it changes, set to "active", set
  //                                 neighbors to "neighbor_is_active" if it did
  //                                 not change, set to "inactive"
  ...
  if (disableComputationWhenStatesAreCloseToEquilibrium_) {
    ...
    if (fiberPointBuffersStatesAreCloseToEquilibrium_[pointBuffersNo] !=
        inactive) {
      // check if it now was indeed inactive
      // loop over all states
      for (int stateNo = 0; stateNo < nStates; stateNo++) {
        // compute relative change
        const Vc::double_v newValue =
            fiberPointBuffers_[pointBuffersNo].states[stateNo];
        ...
        // if maximum relative change in the pointBuffer (vector of size
        // Vc::double_v::size()) is higher than the tolerance
        if (changeValue > 1e-6)
        // if (Vc::any_of(Vc::abs((newValue - oldValue) / newValue) > 1e-5))
        // if (true)
        {
          // LOG(INFO) << "point " << pointBuffersNo << " state " << stateNo <<
          // " " << oldValue << "->" << newValue << ": " << relativeChange << "
          // " << changeValue;

          statesAreAtEquilibrium = false;
          break;
        }
        /*LOG(INFO) << "(active and no change), pointBuffersNo " <<
          pointBuffersNo << ", state " << stateNo
          << " newValue: " << newValue << ", oldValue: " << oldValue
          << ", changeValue: " << changeValue << ", relativeChange: " <<
          relativeChange;*/
      }
    }

    // if the current point is inactive but was computed because the neighbours
    // were not inactive, check if this condition still holds
    if (fiberPointBuffersStatesAreCloseToEquilibrium_[pointBuffersNo] ==
            neighbor_is_active &&
        statesAreAtEquilibrium) {
      // check if one of the neighbours is still not inactive, if it is not, set
      // own point to inactive
      if (pointBuffersNo > 0)
        if (fiberPointBuffersStatesAreCloseToEquilibrium_[pointBuffersNo - 1] !=
            active) {
          if (fiberPointBuffersStatesAreCloseToEquilibrium_[pointBuffersNo] !=
              inactive) {
            fiberPointBuffersStatesAreCloseToEquilibrium_[pointBuffersNo] =
                inactive;
            nFiberPointBufferStatesCloseToEquilibrium_++; // this counter allows
                                                          // to disable 1D
                                                          // computations when
                                                          // no fiber on the
                                                          // current rank is
      ...
      if (statesAreAtEquilibrium) {
        ...
        if (pointBuffersNo > 0) {
          if (fiberPointBuffersStatesAreCloseToEquilibrium_[pointBuffersNo -
                                                            1] == inactive) {
            nFiberPointBufferStatesCloseToEquilibrium_--;
            fiberPointBuffersStatesAreCloseToEquilibrium_[pointBuffersNo - 1] =
                neighbor_is_active;
          }
        }
...
bool FastMonodomainSolverBase<nStates, nAlgebraics,
                              DiffusionTimeSteppingScheme>::
    isEquilibriumAccelerationCurrentPointDisabled(bool stimulateCurrentPoint,
                                                  int pointBuffersNo) {
  if (disableComputationWhenStatesAreCloseToEquilibrium_) {
    ...
    // do not compute current point if the states are at the equilibrium anyways
    // and won't change by the computation
    if (fiberPointBuffersStatesAreCloseToEquilibrium_[pointBuffersNo] ==
        inactive) {
      return true;
    }
